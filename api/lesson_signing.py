...
def _enrollment_part(enrollment_id: str | uuid.UUID | None) -> str:
    if enrollment_id is None:
        return ""
    text = str(enrollment_id).strip()
    return text if text else ""


def sign_lesson_access(
    child_id: str | uuid.UUID,
    slug: str,
    exp: int | None = None,
    *,
    ...
    cid = str(child_id)
    eid = _enrollment_part(enrollment_id)
    msg = f"{cid}:{slug}:{eid}:{exp}" if eid else f"{cid}:{slug}:{exp}"
    sig = hmac.new(_secret(), msg.encode("utf-8"), hashlib.sha256).hexdigest()
    return exp, sig


def verify_lesson_access(
    child_id: str | uuid.UUID,
    slug: str,
    exp: int,
    sig: str,
    *,
    ...
    if int(time.time()) > int(exp):
        return False
    expected = sign_lesson_access(child_id, slug, exp, enrollment_id=enrollment_id)[1]
    return hmac.compare_digest(expected, sig)


def build_lesson_url(
    child_id: str | uuid.UUID,
    slug: str,
    *,
    enrollment_id: str | uuid.UUID | None = None,
) -> str:
    exp, sig = sign_lesson_access(child_id, slug, enrollment_id=enrollment_id)
    cid = str(child_id)
    url = f"{PUBLIC_BASE_URL}/lesson/{slug}?child={cid}&exp={exp}&sig={sig}"
    eid = _enrollment_part(enrollment_id)
    if eid:
        url = f"{url}&enrollment={eid}"
    return url
