...
SUBJECT_QUIZ_AUTO = "Доступ к уроку и PDF-чек-лист от Читательства"
SUBJECT_QUIZ_EARLY = "Пробный урок и PDF-чек-лист от Читательства"
    ...
    *,
    ...
    if trial and lesson_url:
        gift_line = (
            ...
            "Личное письмо от основателя с рекомендациями придёт чуть позже — отдельно."
        )
    elif trial:
        gift_line = (
            f"Вы выбрали бесплатный урок «{trial}». "
            "Ссылка на доступ должна быть в этом письме или на странице после квиза. "
            "Личное письмо от основателя с рекомендациями придёт чуть позже."
        )
    else:
        gift_line = (
            "Доступ к платформе и бесплатному уроку открываем сразу после квиза — "
            "если ссылка не пришла в этом письме, она будет на странице после отправки. "
            "Личное письмо от основателя с рекомендациями придёт чуть позже — отдельно."
        )
