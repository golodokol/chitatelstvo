...
# Номера = сюжет уроков (не порядок точек на тропе сверху вниз).
EARLY_STORIES_MAP_PADS: tuple[tuple[float, float], ...] = (
    (0.160, 0.338),  # 1 — кот в коробке (прихожая)
    (0.333, 0.326),  # 2 — дождь за окном
    (0.608, 0.362),  # 3 — мяч в гостиной
    (0.862, 0.350),  # 4 — книжки · память
    (0.151, 0.688),  # 5 — мокрый кот (ванная)
    (0.376, 0.740),  # 6 — кот и плед (спальня)
    (0.633, 0.731),  # 7 — Словик пришёл (крыльцо)
    (0.857, 0.734),  # 8 — Словик дома (нижняя библиотека)
)
