...
$cmd = "set -e; cd $dir; git pull origin main; cat db/migrations/002_tale_ratings.sql | docker compose exec -T postgres psql -U literary -d literary_school; cat db/migrations/003_chest_claims.sql | docker compose exec -T postgres psql -U literary -d literary_school; docker compose restart api worker; mkdir -p $assets; cp docs/tilda-zero-main/chit-quiz.js docs/tilda-zero-main/chit-quiz.css $assets/; cp docs/images/gamify-badge-* $assets/ 2>/dev/null || true; bash scripts/rename_assets_lower.sh; echo DEPLOY_OK"
